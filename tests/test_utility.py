...
import knowknow.utility as knowwnow


class Test(TestCase):
    def test_comb(self):
        self.assertEqual(knowwnow.comb('q.w.e', 'e.r.t'), 'e.q.r.t.w')

    def test_gen_tuple_template(self):
        template = knowwnow.gen_tuple_template(tuple('qwer'))
    ...
    def test_make_cross(self):
        ...
        namedtuple = knowwnow.make_cross(k___v)
        old_cross = knowwnow.make_cross_kwargs(k___v)
    ...
    def test_named_tupelize(self):
        ...
        tupelized = knowwnow.named_tupelize(d, ctypes)
        self.assertEqual(len(d), len(tupelized))
        for key in d:
            self.assertTrue(
                knowwnow.make_cross(dict(zip(sorted(ctypes.split('.')), key))) in tupelized)
